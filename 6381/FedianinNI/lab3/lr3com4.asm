...
STRerror				db		'Can`t free memory',10,13,'$'
Error2					db		'Error while allocating memory. Size of a free memory: $'
STRerror2               db      '    h',10,13,'$'
...
FreeMemSucs 			db 		'Memory is free sucsesfull',10,13,'$'
;ПРОЦЕДУРЫ
;----------------------------------------------------- 
...
EM			ENDP
;--------------------------------------------------
; Освобождение памяти и запрос 64 Кб
FM			PROC	near
			push 	ax
			push	bx
			push	dx
			mov 	bx,1000h
			mov 	ah,48h
			int 	21h ; Запросили 64 Кб памяти
			jnc 	NoError
			mov 	dx, offset Error2
			call 	PRINT_A_STR
			mov 	di, offset STRerror2+3
			mov 	ax,bx
			call 	WRD_TO_HEX
...
ThroughputСycle:			
			...
			mov 	al,' '
			mov		ah,' '
			mov 	si,offset STRcontrolBlocks ; Чистка строки
			mov 	[si+23],ax
			mov 	[si+25],ax
			mov 	[si+27],ax
			mov 	[si+29],ax
