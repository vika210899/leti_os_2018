...
MCB 			db '|             |            |             |            |            |', 0DH, 0AH, '$'
Line 			db '--------------------------------------------------------------------', 0DH, 0AH, '$'
Message 		db 'Error of memory allocation!', 0DH, 0AH, '$'
...
BEGIN:
	...
	int 21h ;AX = код ошибки если CF установлен
			;BX = размер доступной памяти в параграфах (если памяти нехватает)
			;AX = сегментный адрес распределенного блока (если нет ошибок)
	jc CF_1 ;если CF=1, значит ошибка
	jmp CF_0 ;если нет ошибок
CF_1:
	mov dx,offset Message ;вывод сообщения об ошибке выделения памяти
	call PRINT
CF_0:
