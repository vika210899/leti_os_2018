...
CHECK_MEMORY PROC near
	...
	jnc cm_mem_no_err1
		; Вывод ошибки и количества свободной памяти, если получить не удалось
	...
	div bx
	inc ax
	add ax,040h ; 200 слов на PSP
	add ax,020h ; 100 слов
	mov bx,ax
	mov ah,4Ah
	int 21h
...
; В cx хранится номер обрабатываемого MCB(используется для вывода)
PRINT_MCB_INFO PROC near
	...
	mov di,offset STRMCBINFO2+11 ; ADDR
	mov ax,es:[01h]
	call WRD_TO_HEX
