...
str_error db 'Error with allocation memory!!1!', 0DH,0AH,'$'
...
PRINT_MCB_LIST_CURRENT PROC near
	...
	cmp ax,0A000h ; Проверяем на переполнение, если да, то выводим предупреждение и заверщаем работу
	jb overflowcheck
		mov dx, offset str_overflow
		call PRINT_DX
		xor AL,AL
		mov AH,4Ch
		int 21H
