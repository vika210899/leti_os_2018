...
DOST_MEM_STR db 'ÑÆ·‚„Ø≠†Ô Ø†¨Ô‚Ï:'
DOST_MEM_STR_ db '       °†©‚',0DH,0AH,'$'
RASS_MEM_STR db 'ê†·Ë®‡•≠≠†Ô Ø†¨Ô‚Ï:'
RASS_MEM_STR_ db '      ä°†©‚',0DH,0AH,'$'
BU_MEM_STR 	db 'Ä§‡•·· Ç´†§•´•Ê   ê†ß¨•‡   ç†®¨•≠Æ¢†≠®•',0DH,0AH,'$'
BU_MEM		db '                             $'
STRENDL db 0DH,0AH,'$'
ERROR_STR	db 'éòàÅäÄ',0DH,0AH,'$'
...
MCB PROC
	mov bx,0A000h
	mov ax,offset ENDD ; –ø—Ä–∏–±–∞–≤–ª—è–µ–º —Ä–∞–∑–º–µ—Ä —Å–µ–≥–º–µ–Ω—Ç–∞ TESTPC
	mov bl,10h
	div bl
	xor ah,ah
	add ax,1
	...
	mov bx,cs
	add ax,bx ; –ø—Ä–∏–±–∞–≤–ª—è–µ–º cs
	mov bx,es
	sub ax,bx ; –≤—ã—á–∏—Ç–∞–µ–º PSP
	mov al,0
	mov ah,4Ah
	int 21h
	jnc ERROR
